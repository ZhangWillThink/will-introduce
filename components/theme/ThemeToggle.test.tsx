...
vi.mock('./theme-scene', async () => {
  const actual =
    await vi.importActual<typeof import('./theme-scene')>('./theme-scene')
...
describe('ThemeToggle', () => {
  ...
  test('resolvedTheme 为 dark 时显示切换到亮色主题', () => {
    ...
    expect(
      screen.getByTestId('theme-toggle-sun-icon').getAttribute('aria-hidden'),
    ).toBe('true')
    expect(
      screen.getByTestId('theme-toggle-moon-icon').getAttribute('aria-hidden'),
    ).toBe('true')
  })
  ...
  test('reduced motion 时不再为图标层传入旋转和缩放动画', () => {
    ...
    const transitions = motionDivCalls
      .map((props) => props.transition)
      .filter(Boolean)
    expect(
