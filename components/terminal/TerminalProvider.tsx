...
import type { TerminalContextType, CardType, OutputItem } from "@/types/terminal";
...
export function TerminalProvider({ children }: TerminalProviderProps) {
  ...
  const { addToHistory, resetHistoryIndex } = useCommandHistory();
  ...
  const executeCommand = useCallback(
    (command: string) => {
      ...
      // 记录命令到输出
      setOutput((prev) => [...prev, { command: trimmed, timestamp: Date.now(), type: "command" }]);
