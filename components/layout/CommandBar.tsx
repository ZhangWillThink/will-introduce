  ...
  const findCommand = useCallback(
    (name: string): CommandDef | undefined => {
      ...
      return commands.find(
        cmd => cmd.name.toLowerCase() === lowerName || cmd.aliases?.some(a => a.toLowerCase() === lowerName)
      )
    },
    [commands]
  )
    ...
    addOutput({
      ...
      content: (
        <div className="space-y-3">
          <p className="text-muted-foreground">可用命令（输入 'help &lt;command&gt;' 查看详细信息）</p>
          {Object.entries(categories).map(([category, cmds]) => (
            <div key={category}>
              <p className="text-blue-400 mb-1 text-xs uppercase tracking-wider">{category}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                {cmds.map(cmd => (
                  <div key={cmd.name} className="font-mono text-xs">
                    <span className="text-emerald-400">{cmd.name}</span>
                    {cmd.aliases && <span className="text-muted-foreground"> ({cmd.aliases.join(', ')})</span>}
          ...
          <p className="text-muted-foreground text-xs pt-2 border-t border-border/30">
        ...
        <div className="space-y-1">
          {skills.map(skill => (
            <div key={skill.name} className="flex items-center gap-2">
              <span className="font-mono text-xs w-20">{skill.name}</span>
              ...
              <span className="font-mono text-xs text-muted-foreground w-10">{skill.level}%</span>
            </div>
          ))}
        </div>
      ),
      ...
      content: (
        <div className="space-y-2">
          <div className="border-l-2 border-blue-500 pl-3">
            <p className="text-emerald-400 font-mono text-sm">📁 corporate-site/</p>
            <p className="text-foreground text-xs mt-1">企业官网开发 - GSAP · React</p>
            <p className="text-muted-foreground text-xs">高质量动画交互，品牌展示站点</p>
          </div>
          <div className="border-l-2 border-violet-500 pl-3">
            <p className="text-emerald-400 font-mono text-sm">📁 ai-video-library/</p>
            <p className="text-foreground text-xs mt-1">AI 视频素材库 - Node.js · Vector Search</p>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="border-border/70 bg-background/95 backdrop-blur-md border-t"
          >
            <div className="mx-auto max-w-4xl max-h-[60vh] flex flex-col">
              {/* 终端标题栏 */}
              <div className="border-border/50 flex items-center justify-between border-b bg-card/30 px-4 py-2">
                <div className="flex items-center gap-2">
                  <Terminal className="text-blue-400 h-4 w-4" />
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {outputs.length === 0 && (
                  <div className="text-muted-foreground text-xs space-y-1">
                    <p>欢迎来到终端！输入 <span className="text-blue-400 font-mono">help</span> 查看可用命令。</p>
                    <p className="text-muted-foreground/70">
                      或使用 <span className="font-mono">go skills</span>、<span className="font-mono">open github</span> 等命令快速导航。
                    </p>
                  </div>
                )}
              ...
              {/* 输入区域 */}
              <div className="border-border/50 border-t bg-card/20 p-4">
                <div className="flex items-center gap-2">
                  <span className="text-blue-400 font-mono text-sm">{'>'}</span>
                  <input
                    ref={inputRef}
