...
export function CommandBar() {
  ...
  const commands: CommandDef[] = [
    ...
    {
      name: "whoami",
      description: "滚动到首屏并提示（详情只在页面内维护）",
      category: "info",
  ...
  const handleOpen = (args?: string) => {
    ...
    const links: Record<string, string> = {
      ...
      email: "mailto:zwillthink@163.com",
      rings: "mailto:zwillthink@163.com?subject=rings-cli%20project",
      "rings-cli": "mailto:zwillthink@163.com?subject=rings-cli%20project",
      lunana: "mailto:zwillthink@163.com?subject=Lunana%20project",
      adtensor: "mailto:zwillthink@163.com?subject=adtensor%20project",
    };
    ...
    if (target && links[target]) {
      ...
      addOutput({
        type: "error",
        content: `用法：open <github|email|rings-cli|lunana|adtensor>\n未知目标：${args || "(未指定)"}`,
      });
    }
  };
  ...
  const handleWhoami = () => {
    ...
    addOutput({
      type: "success",
      content: `${siteIdentity.displayName} · ${siteIdentity.roleShort} — 完整介绍见首屏「个人介绍」，请勿与下方技能表重复赘述。`,
    });
  };
  ...
  const handleSkills = () => {
    ...
    addOutput({
      type: "success",
      content: "已滚动到技能区「Capabilities」，技术与熟练度仅以该表为准。",
    });
  };
  ...
  const handleContact = () => {
    ...
    addOutput({
      type: "success",
      content: "已滚动到「联系方式」——邮箱 / 电话 / GitHub 仅以该表为准。",
    });
  };
  ...
  const handleFocus = () => {
    ...
    addOutput({
      type: "success",
      content: "已滚动到「当前关注方向」，与首屏引言分工、不重复罗列。",
    });
  };
  ...
  const handleCat = (args?: string) => {
    ...
    const files: Record<string, string> = {
      "about.txt":
        "(见页面 #hero) " +
        `${siteIdentity.displayName} · ${siteIdentity.roleShort} — ${siteIdentity.tagline}`,
      "skills.json": "(见页面 #skills Capabilities)",
      "focus.md": "(见页面 #focus Tracking)",
      "contact.log": "(见页面 #contact)",
      "projects/": "(见页面 #projects)",
    };
    if (files[key]) {
      addOutput({ type: "code", content: files[key] });
    } else {
      addOutput({ type: "error", content: `cat: ${key}: 文件不存在` });
    }
  };
