...
describe('ContactLog', () => {
  test('以 definition list 结构渲染 3 条联系项并保留正确链接', () => {
    ...
    expect(
      emailTerm?.parentElement?.querySelector('dd')?.textContent,
    ).toContain('zwillthink@outlook.com')
    expect(
      githubTerm?.parentElement?.querySelector('dd')?.textContent,
    ).toContain('@ZhangWillThink')
    expect(
      locationTerm?.parentElement?.querySelector('dd')?.textContent,
    ).toContain('Beijing, CN')
    expect(emailLink.getAttribute('href')).toBe('mailto:zwillthink@outlook.com')
    expect(githubLink.getAttribute('href')).toBe(
      'https://github.com/ZhangWillThink',
    )
