...
describe('FocusAreas', () => {
  test('以 4 个 listitem 渲染关注方向并保持标题与说明同项配对', () => {
    ...
    expect(
      within(websiteUpgradeItem as HTMLElement).getByText(
        '在保证加载速度的前提下，持续优化品牌叙事、交互动画和信息结构。',
      ),
    ).not.toBeNull()
    expect(
      within(observabilityItem as HTMLElement).getByText(
        '建立 Web Vitals、日志与告警闭环，用真实数据持续驱动体验优化。',
      ),
    ).not.toBeNull()
